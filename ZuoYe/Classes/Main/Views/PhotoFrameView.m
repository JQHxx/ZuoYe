...
-(NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    return photosArr.count==kMaxPhotosCount?kMaxPhotosCount:photosArr.count+1;
}

-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ImageCollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ImageCollectionViewCell" forIndexPath:indexPath];
    if (indexPath.row == photosArr.count&&photosArr.count<kMaxPhotosCount) {
...
-(void)photoFrameVIewDidSelectPhotoAction:(UIButton *)sender{
    if (sender.tag == photosArr.count&&photosArr.count==kMaxPhotosCount) {
...
-(void)setupImageCollectionView{
    ...
    CGFloat orginX = 0.0;
    if (_isSetting) {
        itemW = (self.width - 32.0-16.0)/3.0;
        orginX = 17.0;
    }else{
        itemW = (self.width -4*5.0)/3.0;
        orginX = 5.0;
    }
    ...
    layout.sectionInset = UIEdgeInsetsMake(5, 5, 5, 0);
